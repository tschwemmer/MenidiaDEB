...
% basis, and ignoring the embryonic stage. This is the simplified model in
% compound parameters following Jager (2020),
% <https://doi.org/10.1016/j.ecolmodel.2019.108904> (without toxic
% effects).
%
...
X0mat = [1     % the scenarios
         10    % initial shell length in mm (not used as par.L0 is defined)
...
par.L0   = [12.8  1 0 1e6 1]; % body length at start experiment (mm)
par.Lp   = [22    1 0 1e6 1]; % body length at puberty (mm)
par.Lm   = [35    1 0 1e6 1]; % maximum body length (mm)
par.rB   = [0.03  1 0 1e6 1]; % von Bertalanffy growth rate constant (1/d)
...
par.Lj   = [0 0 0  10 1]; % body length at end acceleration (mm)
...
% calc_proflik(par_out,{'rB','Lm'},opt_prof,opt_optim);  % calculate a profile
