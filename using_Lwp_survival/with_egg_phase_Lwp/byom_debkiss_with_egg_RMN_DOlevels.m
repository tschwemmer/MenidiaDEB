...
% Sources: 6-110 dpf is Murray and Baumann 2020, 323 is Concannon et al 2021
DATA{2} = [1       8        4 
           6       5.1      4.6
           6       5.5      4.5
           16      8.9      NaN
...
X0mat = [8       4    % the scenarios
         8       4       % initial oxygen level in mg/L
         0.001   0.001   % initial shell length in mm
         0       0       % initial cumul. repro
         0.15    0.15    % initial weight of buffer in egg
         1.0     1.0];   % initial value for survival
...
glo.dV    = 0.288;   % dry weight density (used in call_deri.m)
...
par.sJAm = [0.35     0 0 1e6]; % specific assimilation rate 
...
par.Lwp  = [115      0 0 1e6]; % total length at puberty - fix, based on data, in this model it is when they start producing eggs.
par.yAV  = [0.8      0 0 1];   % yield of assimilates on volume (starvation)
par.yBA  = [0.95     0 0 1];   % yield of egg buffer on assimilates (repro)
par.yVA  = [0.5      1 0 1];   % yield of structure on assimilates (growth)
...
par.mu_emb = [0.07   0 0 1e6]; % mortality rate for embryos
par.mu_lar = [0.02   0 0 1E6]; % mortality rate for larvae
...
glo.ylab{1} = 'oxygen (mg/L)';
glo.ylab{2} = 'total length (mm)';
glo.ylab{3} = 'cumulative reproduction (eggs)';
glo.ylab{4} = 'egg buffer (mg)';
glo.ylab{5} = 'survival';
...
opt_plot.legsup  = 0; % Added by TS to omit legend

% optimise and plot (fitted parameters in par_out)
par_out = calc_optim(par,opt_optim); % start the optimisation
calc_and_plot(par_out,opt_plot); % calculate model lines and plot them

%calc_proflik(par_out,'yVA',opt_prof); % likelihood profiles
