...
DO = X(1); % state 1 is dissolved oxygen
WV = X(2); % state 1 is the structural body mass
% cR = X(3); % state 2 is the cumulative reproduction (not used in this function)
WB = X(4); % state 3 is the egg buffer of assimilates
SS = X(5);  % state 4 is survival function
...
% Apply stress function to yVA below an oxygen threshold 
% s = SP * DO               % calculating stress from stress param & DO
% if DO > 6                 % if DO is above embryo threshold
%   WVb = WB0 * yVA * kap   % body mass at birth is calculated this way
% else
%   WVb = WB0 * (1-s)*yVA * kap  % need to define s 
...
% Apply stress function to yVA below an oxygen threshold 
% if DO > 6                  % if DO is above embryo threshold
%   JV = yVA * (kap*JA-JM)   % body mass at birth is calculated this way
% else
%   JV = (1-s)*yVA * (kap*JA-JM)  % need to define p and s 
...
dDO = 0;              % Dissolved oxygen remains constant

dX = [dDO;dWV;dcR;dWB;dS];       % collect all derivatives in one vector
