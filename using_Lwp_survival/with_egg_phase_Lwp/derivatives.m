...
% K     = par.K(1);     % parameter to control shape of correction factor curve
% DOc   = par.DOc(1);   % parameter to control DO level below which c=0
...
% Define correction factor c for sJAm and yVA that reaches zero at DOc

% DO = read_scen(-1,c,t,glo);     % use read_scen to derive actual exposure (DO) concentration at time t
% 
% %DOc = 1.7;                         % set critical DO threshold below which the assimilation or yield is 0
% 
% if DO > DOc
%     %c = (DO - DOc)/(DO - DOc + K);       % Option 1 to calculate correction factor
%     c = (K * (DO - DOc)) / (1 + K * (DO - DOc));          % Option 2 to calculate correction factor
% else
%     c = 0;                               % the correction factor will then be multiplied by the parameter wherever it shows up in equations
% end
% 
% 
% % Define correction factor c1 for sJM, mu_emb, and mu_lar 
% 
% c1m = 10;                       % upper limit of c1, which does not affect shape of rest of the curve
% 
% c1 = min([1/c c1m]);     % c1 will be calculated by the function unless it is greater than c1m


% % Simulate changing parameter with a dummy correction factor d
% if DO == 7.7
%     d = 1;
% elseif DO == 4.2
%     d = 0.8;
...
% Model equations with correction factor applied to alter the parameter(s) of interest
...
JA = f * sJAm * L^2;          % assimilation - Eq. 2.7
